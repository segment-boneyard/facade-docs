...
angular
	...
	.controller('facadeController', ['$http', '$state', '$timeout', 'data', 'contents', 'coordinates', function($http, $state, $timeout, data, contents, coordinates){
		...
		function initialize (){
			if (coordinates.category != 'intro') {
				vm.ifIntroPage = false;
				createGithubURL();
			}
		...
		function toggleCollapse (categoryId){
			return vm.contents[categoryId].active = !vm.contents[categoryId].active;
		}
		...
		function createGithubURL (){
			// facade.js#L51-L63
			var link = coordinates.category + '.js';
			if (coordinates.category == 'common') link = 'facade.js';
			if (coordinates.section == 'action' || coordinates.section == 'type') link = 'track.js';
			if (vm.data.url.length == 2) {
				vm.data.url = link + '#L' + data.url[0] + '-L' + data.url[1];
			} else {
				vm.data.url = link + '#L' + data.url[0];
			}
		}
	}]);
